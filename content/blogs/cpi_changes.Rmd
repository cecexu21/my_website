...
Then, we decide to do some data cleaning work to use lubridate() like ymd() or year() to create new variables and add to the original dataset.

Most importantly, we should calculate the yoy change through a function that is 
yoy(t)=value(t)/value(t-1)*100%

Then we will use ifelse function to create a new variable called positive, if yoy(t)>0, then it should be listed as postive, vice versa. Most importantly, we need to colour the spots according to whether it's postive or not. 

However,

```{r cpi_plot,fig.width=8, fig.align='center',cache=TRUE}
...
During this part, we will focus on some part
